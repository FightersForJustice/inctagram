...
const regDummyData = require('../fixtures/regDammyDate.json')
...
describe('Registration Page', () => {
  ...
  it('Registration with incorrect userName format', () => {
    cy.wrap(regTestData.invalidName).each(($item, index) => {
      cy.get('[name = userName]').clear().type($item)
      ...
      if (index > 0) {
        ...
        if ($item.length < 6) {
          cy.get(regSelectors.RegError).contains('Username must be longer than or equal to 6 characters')
