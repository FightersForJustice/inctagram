...
describe('Login Page', () => {
  ...
  describe('Profile Test', () => {
    ...
    it('Profile Settings validation AboutMe', () => {
      ...
      profilePage.elements.profileSettings().click()
      cy.get('#aboutMe').clear().type(randomText)
      profilePage.elements.errorMessage().contains('The number of characters must not exceed 200')
    ...
    it.only('Upload photo check', () => {
      ...
      cy.contains('General Information').should('be.visible')
      profilePage.elements.addPhotoButton().click()
      profilePage.elements.selectFromComputerButton().should('be.visible').should('contain', 'Select from Computer')
      ...
      cy.fixture('image.jpg').then((fileContent) => {
        cy.get('input[type="file"]').then(($input) => {
          ...
          const testFile = new File([blob], 'image.jpg', { type: 'image/jpeg' })
